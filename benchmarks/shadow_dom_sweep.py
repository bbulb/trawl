"""C6 follow-up — Playwright shadow-DOM unwrap spike.

Measures whether inlining shadow-root code content for custom elements
in ``SHADOW_DOM_UNWRAP_TAGS`` fixes the MDN `claude_code_mdn_fetch_api`
assertion without regressing other ``code_heavy_query`` patterns.

Two modes:

    - ``shadow_dom_off`` — baseline: ``TRAWL_SHADOW_DOM_UNWRAP=0``
                            (current default on develop)
    - ``shadow_dom_on``  — experiment: ``TRAWL_SHADOW_DOM_UNWRAP=1``

Pre-registered gates (design doc
``docs/superpowers/specs/2026-04-20-playwright-shadow-dom-design.md``):

    (a) adopt if MDN flips to PASS AND net_assertion_delta >= +1 AND
        flipped_to_fail == 0 AND parity 15/15 under shadow_dom_on.
    (b) reject if MDN doesn't flip OR net_delta < +1.
    (c) parity regression → revert.

Invoke:
    python benchmarks/shadow_dom_sweep.py --dry-run
    python benchmarks/shadow_dom_sweep.py                 # full sweep
    python benchmarks/shadow_dom_sweep.py --iterations 3
    python benchmarks/shadow_dom_sweep.py --skip-parity
    python benchmarks/shadow_dom_sweep.py --only <pattern_id>

Writes ``benchmarks/results/shadow-dom-sweep/<ts>/``:
...
Exit code:
    ...
    1  — measurement failure (>25% patterns errored)
...
RESULTS_ROOT = BENCH_DIR / "results" / "shadow-dom-sweep"
...
def _set_mode_env(mode: str) -> None:
    ...
    if mode == "shadow_dom_off":
        os.environ["TRAWL_SHADOW_DOM_UNWRAP"] = "0"
    elif mode == "shadow_dom_on":
        os.environ["TRAWL_SHADOW_DOM_UNWRAP"] = "1"
    else:
        raise ValueError(f"unknown mode {mode!r}")
    # Each mode changes what Playwright returns, so we must skip the
    # per-fetch cache across modes (same URL, different HTML).
...
def _run_once(pattern: Any, iteration: int, verbose: bool) -> RunOutcome:
    ...
    if verbose:
        ...
        print(
            ...
            f"top1={outcome.top1_sig} n_total={outcome.n_chunks_total} {score_s}",
            file=sys.stderr,
        )
    return outcome
...
def _diff_vs_baseline(baseline: dict[str, Any], experiment: dict[str, Any]) -> dict[str, Any]:
    ...
    n_chunks_changed: list[tuple[str, int, int]] = []
    ...
    for e_pat in experiment["patterns"]:
        ...
        b_total = (b_pat.get("n_chunks_total") or [None])[0]
        e_total = (e_pat.get("n_chunks_total") or [None])[0]
        if b_total is not None and e_total is not None and b_total != e_total:
            n_chunks_changed.append((e_pat["id"], b_total, e_total))
    ...
    return {
        ...
        "n_chunks_total_changed": n_chunks_changed,
        "net_assertion_delta": experiment["assertion_pass"] - baseline["assertion_pass"],
    }
...
def _run_parity(mode: str, repo_root: Path, verbose: bool) -> dict[str, Any]:
    ...
    if mode == "shadow_dom_off":
        env["TRAWL_SHADOW_DOM_UNWRAP"] = "0"
    elif mode == "shadow_dom_on":
        env["TRAWL_SHADOW_DOM_UNWRAP"] = "1"
    env["TRAWL_FETCH_CACHE_TTL"] = "0"
...
def _decide_gate(
    ...
    par_exp = parity.get("shadow_dom_on") or {}
    ...
    if not exp_par_ok and par_exp:
        return {
            ...
            "reason": "shadow_dom_on parity regressed",
        }

    diff = diffs.get("shadow_dom_on") or {}
    net_delta = diff.get("net_assertion_delta", 0)
    flipped_to_fail = diff.get("flipped_to_fail", []) or []
    flipped_to_pass = diff.get("flipped_to_pass", []) or []
    mdn_flipped = "claude_code_mdn_fetch_api" in flipped_to_pass

    if net_delta >= 1 and len(flipped_to_fail) == 0 and mdn_flipped:
        return {
            "outcome": "a_adopt",
    ...
    return {
        ...
        "flipped_to_pass": flipped_to_pass,
        "flipped_to_fail": flipped_to_fail,
        "mdn_flipped_to_pass": mdn_flipped,
        "parity_failed_modes": parity_failed,
        "reason": (
            f"net_delta={net_delta} (need >=1), "
            f"flipped_to_fail={flipped_to_fail} (need ==[]), "
            f"mdn_flipped={mdn_flipped} (need True)"
        ),
    }
...
def _render_report(summary: dict[str, Any]) -> str:
    ...
    lines: list[str] = []
    lines.append("# Shadow DOM unwrap — sweep report")
    lines.append("")
    ...
    lines.append("")
    lines.append("| mode | net Δ | flipped→pass | flipped→fail | top1 changed | n_chunks changed |")
    lines.append("|---|---:|---|---|---:|---:|")
    for mode in summary["modes"]:
        ...
        lines.append(
            ...
            f"{len(d.get('n_chunks_total_changed', []))} |"
        )
    lines.append("")
    if diffs.get("shadow_dom_on", {}).get("n_chunks_total_changed"):
        lines.append("### Pages whose n_chunks_total changed")
        lines.append("")
        lines.append("| pattern | baseline | experiment |")
        lines.append("|---|---:|---:|")
        for pid, b_t, e_t in diffs["shadow_dom_on"]["n_chunks_total_changed"]:
            lines.append(f"| `{pid}` | {b_t} | {e_t} |")
        lines.append("")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--iterations", type=int, default=2, help="Runs per (mode, pattern). Default 2.")
    ...
    if args.skip_parity:
        ...
        for mode in ("shadow_dom_off", "shadow_dom_on"):
