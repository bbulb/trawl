...

{
  ...
  "content_anchors": [
    "<5 to 10 short exact-text snippets (3-10 words each) visibly present in the MAIN CONTENT area of the screenshot. CRITICAL RULES for picking anchors: (1) Pick text from INSIDE paragraphs, code blocks, data cells, or captions — NOT from section headings or titles. Section headings are often duplicated in sidebar tables of contents and breadcrumb navigation, which breaks DOM matching. (2) Each anchor must be text that a human can actually read in the image, not guessed. (3) Spread anchors across the content area, not clustered in one spot. (4) Avoid any text from nav bars, sidebars, footers, headers, cookie banners, or related-content widgets. (5) If the same text appears in both the main content and a sidebar/nav, do NOT use it.>"
  ],
  "noise_labels": [
  ...
  "item_hints": {
    ...
    "example_row_anchors": [
      "<if has_repeating_items: 3-5 anchors that belong to ONE representative row. Otherwise empty array.>"
    ]
  }
}
